...
void UnitManager::updateUnitState()
{
	...
	else
	{
		for (int i = 0; i < receiveMessNum; i++)
		{
			...
			{
				if (receiveMessage.cmd() == MESSAGEATK)//unit1Êî∂Âà∞unit2ÊîªÂáª
				{
					...
					if (_unitIdMap.count(unitId1) > 0)
					{
						...
						if (_unitIdMap.count(unitId2) > 0)//if (_unitManager->_unitIdMap[_attackID]!=nullptr)
						{
							...
							if (_unitIdMap[unitId2]->getCurrentHp() <= 0)
							{
								...
								{
									_unitIdMap[unitId2]->setEnermyId(unitId1);//◊‘∂Øπ•ª˜
									_unitIdMap[unitId2]->setUnderAttack(true);//◊‘∂Øπ•ª˜
		...
		if (unit)
		{
			...
			if (unit->getUnitType() >= 5 && unit->getAttackID() != 0)
			{
				...
				if (_unitIdMap.count(attackId)>0&&_unitIdMap[attackId]->getCamp() == _playerCamp)//_unitIdMap[attackId] != nullptr &&
				{
					...
					_unitIdMap[attackId]->setAutoAttack(true);//Œ¥≤‚ ‘
				}
			}
			if (unit->getUnitType() < 5 && unit->getEnermyId() != 0 && _unitIdMap.count(unit->getEnermyId()) > 0)//Œ¥≤‚ ‘
			{
...
Unit * UnitManager::creatUnit(CampTypes camp, UnitTypes type, const  GridVec2& pos, int id )
{
	...
	{
		if (type > 5)
			SimpleAudioEngine::getInstance()->playEffect("Music/Unit ready.wav");//“Ù–ß
	}
	if(type<5)
		unit->buildingUpdate();
...
//Êú¨Âú∞ÁßªÂä®ÂíåÊîªÂáªÁöÑÊ∂àÊÅØÊÄé‰πàÁîüÊàêÔºüÔºü
void UnitManager::choosePosOrUnit(const GridVec2 & pos)
{
	...
	if (!_selectedUnitID.empty())
	{
		if (idAtPos == 0 || idAtPos == _NO_PASS)
		{
			for (auto unitid : _selectedUnitID)
			{
				if (_unitIdMap[unitid]->getUnitType() >= 5)// «ø…“∆∂Øµ•Œª
				{
		...
		else
		{
			if (_unitIdMap[idAtPos]->getCamp() == _playerCamp
				&&_unitIdMap[idAtPos]->getIsBuilding()==false)//ÈÄâÊã©‰∫ÜÊàëÊñπÁöÑÂçï‰Ωç ÂàôÊòæÁ§∫Ë°ÄÊù?
			{
				deselectAllUnits();
	...
	else 
	{
		if (idAtPos != 0 && idAtPos != _NO_PASS&&
			...
			_unitIdMap[idAtPos]->getCamp() == _playerCamp)//ÈÄâÊã©‰∫ÜÊàëÊñπÁöÑÂçï‰Ωç ÂàôÊòæÁ§∫Ë°ÄÊù?
		{
